...
² **Why ECS?** I've been a loud advocate for services built on AWS API Gateway with application code on Lambda. I've seen these technologies be criminally underused. But there are tradoffs - API Gateway gets pretty expensive when request rates start exceeding hundreds of RPS or so. For high scale web service traffic, ECS with Fargate compute seems low enough in the stack to get some serious cost savings and high enough to not feel like you're working with bare metal. I wanted to try it out. Why not EKS? I'm no infrastructure expert, but I've heard about migrations to EKS taking an eternity. I've heard tons of people complain about how hard it is just to get one service to talk to another service in EKS. Where possible, my philosophy is to embrace off-the-shelf AWS native tools. This approach has served my teams well in the context of very large companies with large cloud budgets, high scale, advantageous pricing, and bespoke support. Compared to EKS, ECS seems far simpler and totally AWS native, and makes it quite easy to orchestrate containers and get services talking to each other—seems to give about 80% of the value of Kubernetes with 20% of the pain for simple backend services. Of course, I believe EKS can be great with support from a killer platform team.
